package jp.techacademy.jouchan.wan.jumpactiongame
...
class GameScreen(private val mGame: JumpActionGame) : ScreenAdapter() {
    ...
    var sound = Gdx.audio.newSound(Gdx.files.internal("flash1.mp3"))
    ...
    init {
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.jouchan.wan.jumpactiongame")
    ...
    private fun createStage() {
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            if (mRandom.nextFloat() > 0.4f) {
                val enemy = Enemy(enemyTexture, 0, 0, 144, 144)
                enemy.setPosition(step.x + mRandom.nextFloat() * 2, step.y + mRandom.nextFloat() * 4)
                mEnemy.add(enemy)
            }
